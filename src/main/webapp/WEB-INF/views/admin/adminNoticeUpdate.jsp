...
<!-- 		        <h5 class="detail-modal-title" id="exampleModalLabel">회원정보</h5> -->
				...
				<c:forEach items="${notice.attachments}" var="attach" varStatus="vs">
					...
					<div class="input-group mb-3" style="padding: 0px;">
						...
						<div class="custom-file">
							...
							<label class="custom-file-label" for="upFile1" id="fname">${attach.originalFilename}</label>
						</div>
						<div id="checkbox">
							<input type="checkbox" name="delFile${vs.count}" id="delFile" value="${attach.attachNo}" /> 
			...
			<c:when test="${notice.attachments[0].attachNo == 0}">
				<c:forEach items="${notice.attachments}" var="attach" varStatus="vs">
					...
					<div class="input-group mb-3" style="padding: 0px;">
						...
						<div class="custom-file">
							<input type="file" class="custom-file-input" name="upFile" id="upFile1" multiple="multiple"> 
							<label sclass="custom-file-label" for="upFile1" id="fname">파일을 선택하세요.</label>
						</div>
					</div>
					...
					<div id="checkbox" style="display: none;">
						<input type="hidden" name="delFile2" id="delFile" value="0" /> 
						<label for="delFile">기존파일삭제</label>
					</div>
				</c:forEach>
			</c:when>
			...
			<c:otherwise>
				<c:forEach items="${notice.attachments}" var="attach" varStatus="vs">
					...
					<div id="checkbox" style="display: none">
						<input type="hidden" name="delFile2" id="delFile" value="1" /> 
						<label for="delFile">기존파일삭제</label>
					</div>
				</c:forEach>
			</c:otherwise>
